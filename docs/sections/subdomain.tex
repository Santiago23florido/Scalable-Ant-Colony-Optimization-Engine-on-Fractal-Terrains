...
\begin{minipage}{0.62\columnwidth}
    ...
    {\footnotesize (c) 160000 ants}
\end{minipage}
\caption{MPI subdomain computation speedup (with and without communication) for selected colony sizes.}
...
\end{minipage}
\caption{Hybrid OMP-only vs MPI-only vs MPI+OpenMP computation-only comparison for selected colony sizes. The left column corresponds to two OpenMP threads per subdomain and the right column corresponds to three OpenMP threads per subdomain.}
